...
| `JWT_SECRET` | Random string of at least 32 bytes | JWT signing key |
| `RIN_GITHUB_CLIENT_ID` | GitHub OAuth Client ID | GitHub OAuth application |
| `RIN_GITHUB_CLIENT_SECRET` | GitHub OAuth Client Secret | GitHub OAuth application |
| `RIN_GITHUB_ADMIN_ID` | Your numeric GitHub user ID | The only creator allowed to log in |

> **Tip**: Generate `JWT_SECRET` with `openssl rand -hex 32`.

#### Login Security Boundary

v1.2 supports creator-only GitHub OAuth. Password login is disabled, and every other GitHub account is rejected by the server after authorization.
...
- **Frontend and browser API origin**: `https://rin-client.pages.dev` (or the name set in `PAGES_PROJECT_NAME`)
- **Backend API**: No independent public URL; it is reachable only through the Pages Functions service binding

The browser uses the Pages address for both UI and `/api/*`. Pages Functions forward dynamic
requests to the backend with the internal `RIN_API` service binding, so login cookies remain
same-origin. The deployment explicitly disables the `workers.dev` entry.

### Step 5: First Login

1. Visit the frontend page
2. Click the login icon in the top right corner
3. Authorize with the GitHub account matching `RIN_GITHUB_ADMIN_ID`
...
3. Fill in the application information:
    ...
    - **Authorization callback URL**: `https://your-pages-domain/api/user/github/callback`
...
1. Confirm all three GitHub OAuth secrets are configured
2. Confirm `RIN_GITHUB_ADMIN_ID` is the numeric ID of the GitHub account being used
3. Confirm the callback URL is `https://your-pages-domain/api/user/github/callback`
4. Check Worker logs for authentication errors, then clear browser cookies and try again
