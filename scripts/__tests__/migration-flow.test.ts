...
describe('迁移流程完整测试', () => {
    it('应该模拟完整的迁移流程', async () => {
        ...
        verbose('\n模拟完整迁移流程:');
        verbose(`  原始 visits 数据: ${visitsData.length} 条记录`);
        ...
        verbose(`  SQL 迁移后: ${visitStatsAfterSQLMigration.size} 个 feed`);
        for (const [feedId, stats] of visitStatsAfterSQLMigration) {
            verbose(`    Feed ${feedId}: PV=${stats.pv}, hll_data='${stats.hll_data}'`);
        }
        ...
        for (const [feedId, stats] of visitStatsAfterSQLMigration) {
            // 检查是否已经有 HLL 数据
            if (stats.hll_data !== '') {
                verbose(`    Feed ${feedId}: 已有 HLL 数据，跳过`);
                continue;
            }
            
            // 获取该 feed 的所有 IP
            const ips = feedGroups.get(feedId);
            verbose(`    Feed ${feedId}: 找到 ${ips.length} 个 visits`);
            ...
            verbose(`    Feed ${feedId}: PV=${ips.length}, UV=${uv}, HLL长度=${hllData.length}`);
        ...
        verbose('\n  迁移结果验证:');
        for (const [feedId, stats] of visitStatsAfterSQLMigration) {
            verbose(`    Feed ${feedId}: PV=${stats.pv}, UV=${stats.uv || 'N/A'}`);
        ...
        verbose(`\n  ✅ 迁移成功!`);
        verbose(`    Feed 1: 期望 UV≈2, 实际 UV=${feed1Stats.uv}`);
        verbose(`    Feed 2: 期望 UV≈3, 实际 UV=${feed2Stats.uv}`);
    });
    ...
    it('应该检查 hll_data 是否为空字符串', () => {
        ...
        verbose('\n检查 hll_data 是否为空:');
        verbose(`  hll_data: '${visitStats.hll_data}'`);
        verbose(`  hll_data !== '': ${hasHLLData}`);
    ...
    it('应该验证 base64 编码不包含单引号', () => {
        ...
        verbose('\n验证 base64 编码:');
        verbose(`  HLL 数据: ${hllData.substring(0, 50)}...`);
        verbose(`  包含单引号: ${hasSingleQuote}`);
